...
└─ images/
   ├─ hero-bg.jpg           ← Woman smiling with green apple (waterfront, golden hour)
   ├─ final-cta-bg.jpg
   ├─ pharmacy-greenwich.jpg
   ├─ monj-logo.png         ← Monj Health logo (used in Advanced pricing card)
...
- [x] `vercel.json` uses `cleanUrls: true` only — no rewrites, no outputDirectory
- [x] `download` blank file present
- [x] `styles.css` present but unused — safe to leave or delete before upload
- [x] Primary CTA: `https://precisiontelemed.com/start-weight-loss-program-tirzepatide/`
- [x] Sermorelin cross-sell: `https://precisiontelemed.com/sermorelin/`
